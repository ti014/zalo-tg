...
test('Telegram group command catalog exposes every supported slash shortcut once', () => {
  ...
  for (const required of [
    ...
    'members', 'recall', 'backup', 'restore', 'login', 'loginweb', 'loginapp',
    'autoreply', 'group_info', 'group_infoall', 'history', 'seed', 'admin', 'update', 'restart',
    'help',
  ]) {
...
test('Telegram private command catalog only exposes the private login flow', () => {
  assert.deepEqual(PRIVATE_BOT_COMMANDS.map(entry => entry.command), [
    'login', 'loginweb', 'loginapp',
  ]);
});
